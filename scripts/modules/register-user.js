...
function onRegisterSubmit() {
	...
	if (DOMnodes.userName.checkValidity() === false) {
		...
		if (FoodPlate.user.age === 'adult') {
			let ageGroup = document.querySelector(
				'#adultAgeGroups span.ui-selectmenu-text'
			).textContent;
			console.log('registered age: ' + ageGroup);
			FoodPlate.user.ageGroup = ageGroup;
		} else if (FoodPlate.user.age === 'child') {
			let ageGroup = document.querySelector(
				'#childAgeGroups span.ui-selectmenu-text'
			).textContent;
			console.log('registered age: ' + ageGroup);
			FoodPlate.user.ageGroup = ageGroup;
		}
...
function confirmUser(user) {
	...
	DOMnodes.registerHeaderH1.innerText =
		FoodPlate.user.userName + "'s Food Plate";
	...
	DOMnodes.welcomeHeader.innerText =
		FoodPlate.user.userName + "'s Food Plate";
}
...
function createConfirmMessage() {
	...
	confirmDiv.classList = 'confirmMessage';
	...
	confirmH2userName.append(
		document.createTextNode(
			'You have successfully registered as: ' + FoodPlate.user.userName
		)
	);
	...
	confirmPage.append(
		document.createTextNode('Your age group is: ' + FoodPlate.user.age)
	);
	...
	confirmGender.append(
		document.createTextNode('Your gender is: ' + FoodPlate.user.userGender)
	);
