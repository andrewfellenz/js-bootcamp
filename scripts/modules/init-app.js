...
function checkUser() {
	...
	console.log(
		'%ccheckUser function called - user registered:',
		'color:green',
		`${FoodPlate.user.registered}`
	);
}
...
function setRegBtn() {
	...
	if (!FoodPlate.user.registered) {
		...
		DOMnodes.regBtn.addEventListener('click', function () {
			...
			$.mobile.navigate('#registerFormPage', {
				transition: 'flip',
				info: 'register user',
			});
			//log button click
			console.log('user clicked register button');
			DOMnodes.registerUserBtn.addEventListener(
				'click',
				onRegisterSubmit
			);
			processForm();
		...
		DOMnodes.regBtn.addEventListener('click', function () {
			...
			$.mobile.navigate('#addFoodPage', {
				transition: 'flip',
				info: 'let user add food',
			});
		});
	}
}
...
function checkTime() {
	...
	FoodPlate.difference = Math.ceil(
		FoodPlate.difference / (1000 * 60 * 60 * 24)
	);
	console.log(
		'difference between register date and check in date is: ' +
			FoodPlate.difference
	);
	if (FoodPlate.difference >= 0) {
		console.log(
			'user has checked in on the same date as registration date'
		);
	} else if (FoodPlate.difference <= -1) {
		console.log(
			'24 hours have passed - clean plate and reset checkin/return dates'
		);
		alert(
			'It has been more than 24 hours since your last check in. Your plate will be reset for today.'
		);
